...
- Slack notification routing is live in code on `origin/main`/VPS at `ff7dd9b`: persisted Slack attempts, readiness checks, route config, and notifications for lead-machine run digests/hot leads, Instantly replies, lease-option inbound leads, SMS replies, and Vapi call events. Activation still requires Slack route channels, bot membership, `SLACK_BOT_TOKEN`, and route channel IDs on the VPS; readiness currently blocks with no post because those env vars are missing.
...
12. Activate Slack routing only after route channels exist, the Ares Slack bot is invited to them, and VPS env has `SLACK_NOTIFICATIONS_ENABLED=true`, `SLACK_BOT_TOKEN`, `SLACK_CHANNEL_LEAD_RUNS`, `SLACK_CHANNEL_HOT_LEADS`, `SLACK_CHANNEL_INSTANTLY_REPLIES`, `SLACK_CHANNEL_LEASE_OPTION_INBOUND`, and `SLACK_CHANNEL_SMS_CALLS`.
...
- Slack readiness exits `2` with `would_post=false` because the VPS has no `SLACK_BOT_TOKEN`, `SLACK_NOTIFICATIONS_ENABLED=true`, or route channel IDs. Slack connector channel search found no existing Ares/lead/Instantly/lease/SMS channels and the connector cannot create channels. No Slack posts, provider sends, or outbound mutations were made.
