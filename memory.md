...
- Back Office Spine v0 landed on `main` at `e898ee0` and the local feature branch was deleted: canonical deal records, lead-to-deal promotion, lane-template tasks/docs/risks, stage blockers, fire-list, Supabase runtime persistence, and a read-only Mission Control Deal Desk. QC `docs/qc/2026-05-16/back-office-spine-v0/`; post-merge verification passed full backend `942`, Mission Control `82`, Mission Control typecheck/build, Trigger typecheck, and diff check. Follow-on hardening landed on GitHub `main` at `709f714` and Docker deployment tracking at `be11aaa`; VPS `100.74.177.6` rebuilt from `be11aaa` with Supabase migration `20260516011000_deal_spine_runtime` applied and live Caddy routes verified.
...
- VPS `100.74.177.6` live Ares wiring as of 2026-05-16: Caddy on port 80 routes API paths to Docker `ares-api` on `127.0.0.1:8000` and UI fallback to `ares-ui` on `127.0.0.1:8080`; `/opt/ares/docker-compose.yml` builds both images from `/opt/ares/Ares`; server worktrees and rebuilt images are at `be11aaa`; Caddy backup before route expansion is `/etc/caddy/Caddyfile.bak.20260516T023712Z`; Caddy routes include `/crm*`, `/deals*`, `/sms-agent*`, and `/voice*`; Supabase migration `20260516011000_deal_spine_runtime` is applied. Verified through Caddy: `/health` 200, `/crm/hubspot/customization` GET 405, `/deals` 200, `/deals/fire-list` 200, and `/mission-control/probate-autopilot/health` 200. Deal endpoints are correct but hydrate Supabase in roughly 10-11s on the VPS, so cache/profile before heavy operator use.
- HubSpot operating spine / agentic company Phases 1-9 are complete with final QC index/readiness artifacts and runbooks under `docs/qc/2026-05-14/` and `docs/runbooks/`.
