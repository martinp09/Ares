...
- `fix/origin-main-hardening-cleanup` in `/tmp/ares-hardening-cleanup` hardens current `origin/main` at `be76288`: Montgomery PublicSearch land-record lookups use a current-day end date, live no-send smoke asserts public case-detail calls, legacy `/crm/hubspot/*` live writes require `operator_approval=true`, Docker deployment files are tracked, and GitHub Actions CI covers backend, Mission Control, Trigger, Docker image, and whitespace gates.
