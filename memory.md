...
- Probate autopilot is a live no-send intelligence/enrichment system: VPS `/opt/ares/Ares` is deployed from `ff7dd9b`, production no-send env preflight was healthy for `limitless/prod` before the Slack deploy, Docker `ares-api` has durable `/var/lib/ares/lead-machine` mounted, and Hermes no-agent cron `815e1261ab2e` is the active no-send CT scheduler/watchdog until Trigger.dev points at an accessible current Ares runtime. Trigger CLI auth is recovered for Hermes and Trigger prod `20260516.2` is deployed with cloud schedule no-op guards unless `ARES_TRIGGER_SCHEDULES_ENABLED=true`; Trigger prod env still points at stale Vercel where protected probate health returned `404`. Dedupe/isolation hardening uses `county_case_sha256_v1` hashed identities scoped by `business_id + environment + source_run_scope`; scheduled payloads are `autonomous`; manual Hermes forced runs use isolated manual state/environment; remote Supabase migration `20260516131500_probate_source_identity_dedupe.sql` is applied; and `NightlyLeadMachineService` reads/writes `public.probate_source_identities` when `LEAD_MACHINE_BACKEND=supabase`. Post-adapter monitor passed 2-day live no-send smoke with 48 source rows/8 keep-now/no sends; Harris live rows are postback-only for case-detail and safely incomplete (`case_detail_postback_only`). QC: `docs/qc/2026-05-16/probate-production-readiness-wrap/`, `docs/qc/2026-05-16/probate-post-adapter-live-no-send-monitor/`, `docs/qc/2026-05-16/probate-dedupe-runtime-isolation/`, `docs/qc/2026-05-16/probate-source-identity-supabase-migration/`, `docs/qc/2026-05-16/probate-source-identity-supabase-adapter/`, and `docs/qc/2026-05-16/slack-routing-vps-deploy/`.
- Slack notification routing is merged to `main` as `ff7dd9b` and deployed on VPS `100.74.177.6`. Remote Supabase migration `20260516012000_slack_notifications.sql` is applied; Slack route channels now exist; the Ares Slack bot is installed with `chat:write` and `chat:write.public`; `/opt/ares/Ares/.env` has `SLACK_NOTIFICATIONS_ENABLED=true`, `SLACK_BOT_TOKEN`, and all five route channel IDs; `ares-api` was recreated and health checks pass. Readiness is configured with all five requested routes `would_post=true`, and Slack `auth.test` returns `ok=true` for team `Ares`. Verification before merge/deploy: focused `224 passed`, full backend `1050 passed`, Trigger typecheck, Docker CI, and `git diff --check` passed. No Slack posts were made.
...
- Slack notification routing is live in code on `origin/main`/VPS at `ff7dd9b`: persisted Slack attempts, readiness checks, route config, and notifications for lead-machine run digests/hot leads, Instantly replies, lease-option inbound leads, SMS replies, and Vapi call events. Route channels, VPS route env, and Ares Slack bot token are configured; readiness is configured and Slack `auth.test` passes. No live test post has been sent; get explicit approval before any manual Slack post.
...
- `/opt/ares/worktrees/ares-main` on `main` now includes the Harris+Montgomery probate autopilot PRD execution as an operational no-send system: code commit `fc99b75` is deployed on the VPS; production no-send env preflight is healthy for `limitless/prod`; scheduled/background intelligence can use live public sources, live public case-detail evidence attempts, and live public CAD/tax/land enrichment; runtime requests still require no-send approvals while all outbound/provider-send paths remain blocked. Post-adapter monitor showed Harris case-detail rows are postback-only and need a dedicated postback client for party/event/document completion. Trigger CLI auth is recovered for Hermes, but Trigger cloud is not authoritative yet: prod version `20260516.2` is deployed with schedules no-op unless `ARES_TRIGGER_SCHEDULES_ENABLED=true`, and Hermes no-agent cron `815e1261ab2e` remains active until Trigger prod points at an accessible current Ares runtime.
...
- VPS `100.74.177.6` live Ares wiring as of 2026-05-16: Caddy is bound to tailnet `100.74.177.6:80` only and routes API paths to Docker `ares-api` on `127.0.0.1:8000` and UI fallback to `ares-ui` on `127.0.0.1:8080`; `/opt/ares/docker-compose.yml` builds both images from `/opt/ares/Ares`; `/opt/ares/Ares` is detached at `origin/main`/`fc99b75`; API/UI containers run non-root with `no-new-privileges`, dropped caps, and loopback-only published ports; Caddy loads the runtime bearer from root-only `/etc/caddy/ares-runtime.env`; `ares-edge-firewall.service` drops public `eth0` access to Caddy/Supabase dev ports; Supabase migration `20260516011000_deal_spine_runtime` is applied; Docker `ares-api` mounts `/var/lib/ares/lead-machine` for probate state/artifacts; production probate env preflight is healthy; `/health` and UI local smokes pass. QC: `docs/qc/2026-05-16/vps-edge-container-hardening/` and `docs/qc/2026-05-16/probate-production-readiness-wrap/`.
- HubSpot operating spine / agentic company Phases 1-9 are complete with final QC index/readiness artifacts and runbooks under `docs/qc/2026-05-14/` and `docs/runbooks/`.
...
1. Continue `docs/superpowers/plans/2026-05-16-textgrid-sms-reply-agent-implementation-plan.md`: public signed TextGrid inbound webhook, SMS-agent job/decision tables, deterministic classifier, draft-only default, Mission Control review surface, Trigger processor, redacted Obsidian/JSONL archive export, and provider activation smoke/runbook are locally scaffolded; the live owned-number activation smoke still requires Martin approval. Keep live auto replies disabled until explicitly approved.
1. Watch the next Hermes no-agent CT scheduler window (or Trigger after auth recovery) for the first post-deploy `limitless/prod` autonomous morning brief; Mission Control currently returns `status=no_data` for prod until that brief exists.
...
4. Keep HubSpot batch writes, Instantly enrollment/send, SMS/Vapi dispatch, paid skiptrace, Slack/provider sends, and deploy as separate explicit approval gates.
...
- Fast-forwarded `/opt/ares/worktrees/ares-main` to `origin/main`/`b74d495`, deployed Trigger prod `20260516.1`, then verified Trigger prod env still points at stale Vercel runtime `https://production-readiness-afternoon.vercel.app` where protected probate health returns `404` while VPS tailnet health is healthy.
