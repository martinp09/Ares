...
- `fix/origin-main-hardening-cleanup` in `/tmp/ares-hardening-cleanup` hardens current `origin/main` at `be76288`: Montgomery PublicSearch land-record lookups use a current-day end date, live no-send smoke asserts public case-detail calls, legacy `/crm/hubspot/*` live writes require `operator_approval=true`, and GitHub Actions CI covers backend, Mission Control, Trigger, and whitespace gates.
- VPS `100.74.177.6` live Ares wiring as of 2026-05-16: Caddy on port 80 routes API paths to Docker `ares-api` on `127.0.0.1:8000` and UI fallback to `ares-ui` on `127.0.0.1:8080`; `/opt/ares/docker-compose.yml` builds both images from `/opt/ares/Ares`; server worktrees now sit at `be76288`, but running Docker images were built on 2026-04-27 and still predate current probate/HubSpot routes.
- HubSpot operating spine / agentic company Phases 1-9 are complete with final QC index/readiness artifacts and runbooks under `docs/qc/2026-05-14/` and `docs/runbooks/`.
...
- Added `.github/workflows/ci.yml` for backend pytest, Mission Control install/test/typecheck/build, Trigger install/typecheck, and whitespace checks. Verification on `be76288`: focused backend `44 passed`; full backend `945 passed`; Mission Control tests `25 files / 82 tests`; Mission Control typecheck/build passed; Trigger typecheck passed; `git diff --check` passed. No VPS mutation, provider send, live smoke, deploy, or Supabase edit was executed.
