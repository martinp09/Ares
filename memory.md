...
- Advanced `/opt/ares/Ares` from `b74d495` to current `origin/main`/`61f18de`, backed up `/opt/ares/Ares/.env`, added explicit SMS reply-agent draft/no-auto-reply env defaults, then rebuilt/recreated Docker `ares-api` and `ares-ui` from that commit.
