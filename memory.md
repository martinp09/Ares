...
- `main` now includes Ares Chief of Staff v3 from `feature/ares-chief-of-staff-v0`: read-only lead desk scoring/bucketing, hot/contact-ready/research/skiptrace/blocked queues, Markdown/JSON/CSV artifacts, opt-in Slack route `chief_of_staff_digest` via `SLACK_CHANNEL_CHIEF_OF_STAFF`, employee-style worklog/priorities/blockers/approval requests, read-only lead-machine health/latest-brief context, PII-redacted Slack text/blocks/payload, manager action items with stable `approve/deny cos_action_...` Slack reply commands, and a protected Trigger-safe employee check-in endpoint `POST /ares-chief-of-staff/internal/check-in` for the new Trigger task/schedule `chief-of-staff-check-in-0815-ct`. The check-in returns sanitized queue/action counts, safety flags, and artifact path maps when artifacts are written. Scheduled Slack delivery is separately gated by `ARES_CHIEF_OF_STAFF_SCHEDULED_SLACK_ENABLED=false` by default. It is separate from Telegram and performs no seller outreach, paid skiptrace, Instantly enrollment, HubSpot/provider writes, SMS/email/Vapi sends, live county/source pulls, approval execution, or buyer blasts. QC: `docs/qc/2026-05-18/ares-chief-of-staff-trigger-check-in/`.
- `main` now includes Ares Appointment Setter v0 and a Chatwoot-inspired Conversation Desk: the SMS decision layer is now a least-privilege real-estate acquisitions ISA with qualification stage/score/bucket/missing-field/next-action outputs, prompt-injection and sensitive-info handoff, nurture/calendar/disqualify recommendations, and hard `manual_control` plus `appointment_setter_paused` kill switches before auto-ack. Mission Control Inbox now exposes Conversation Desk language, conversation owner/acquisition-route/tags context, Appointment Setter score/action/risk/missing-field review, and disabled placeholder controls for takeover/approve/slots/nurture/disqualify until backend command contracts exist. Ares remains source of truth and owner of TextGrid sends, Slack, calendar/Cal.com/Google actions, policy, audit, and provider gates; no live seller/provider/calendar/Slack side effects occurred. QC: `docs/qc/2026-05-18/ares-appointment-setter-conversation-desk/`.
- `main` now includes the first Herrington/Browne email-marketing prep gate: draft-only curative-title/ambiguous-heirship soft-finder campaign docs/backups, local + Instantly verification for five candidate emails, and sanitized QC at `docs/qc/2026-05-18/email-marketing-herrington-browne-prep/`; raw emails/provider responses stay local-only under `/opt/ares/lead-data/email_marketing_herrington_browne_2026-05-18/`. No Instantly upload, activation, or seller email send occurred. A temporary owned-number SMS smoke watcher exists only for Martin's approved number and must not be generalized into global auto-replies. SMS reply-agent v1 now mirrors the Vapi operating pattern: TextGrid transport, Ares-owned contact/message/job/decision state, deterministic safety policy, optional `SMS_AGENT_LLM_REPLIES_ENABLED` copy rewriting only, and Slack/operator handoff boundaries.
...
5. Appointment Setter Slack loop: configure `SLACK_CHANNEL_APPOINTMENT_SETTER`; add Slack-safe hot-seller / needs-Martin-review alerts and approve/reject/takeover commands that write manager intent through Ares only, with no direct provider/calendar/TextGrid calls from Slack.
6. Calendar foundation: add Cal.com-first availability/action adapter behind Ares gates; the Appointment Setter receives sanitized options only and no credentials.
7. SMS/email/outbound gates: keep `SMS_AGENT_AUTO_REPLIES_ENABLED=false`, `APPOINTMENT_SETTER_ENABLED` scoped, provider live-send gates false, and Instantly enrollment/send, SMS/Vapi dispatch, paid skiptrace, HubSpot batch writes, and seller outreach blocked until separately approved.
...
### 2026-05-18 Appointment Setter Conversation Desk

- Added `docs/superpowers/specs/2026-05-18-ares-appointment-setter-conversation-desk-design.md` and `docs/runbooks/ares-appointment-setter-conversation-desk.md` to capture the Chatwoot-inspired Ares-native conversation desk, least-privilege Appointment Setter profile, qualification/disqualification rubric, Slack escalation, future calendar boundaries, and manual takeover/kill-switch rules.
- Extended the SMS reply decision layer into an acquisitions ISA contract with qualification score/bucket/stage/missing-fields/next-action, appointment/nurture/disqualify flags, prompt-injection and sensitive-info risk flags, and `manual_control` / `appointment_setter_paused` gates that force human handoff before `auto_ack`. Service-level regression proves a paused thread cannot send even when live provider sends and auto-ack are enabled.
- Remodeled the Mission Control Inbox into a Conversation Desk: queue language is Chatwoot-style but Ares-native, the thread view shows owner/acquisition route/tags context, and the Appointment Setter review panel surfaces score/action/risk/missing-field state plus disabled takeover/approve/slots/nurture/disqualify controls. Verification: focused backend `48 passed`, full backend `1158 passed`, Mission Control typecheck, `25` files / `85` tests, Vite build, `git diff --check`, and fresh QC PASS review. QC: `docs/qc/2026-05-18/ares-appointment-setter-conversation-desk/`. No seller SMS/email/calls, Slack posts, calendar writes, paid skiptrace, provider writes, global auto-replies, Supabase remote migration, VPS deploy, or Telegram delivery occurred.

### 2026-05-18 Email Marketing Prep + Owned SMS Smoke

- Added a draft-only curative-title/ambiguous-heirship soft-finder email campaign packet and Instantly/local backup exports for the Herrington/Browne test contacts. The copy stays in right-person/paperwork language and avoids ownership, authority, title, tax, legal, and offer claims.
- Ran local syntax/MX/disposable/role checks plus Instantly email verification jobs and Instantly duplicate/lead-membership read probes for five candidate emails. Sanitized repo QC masks emails/hashes them; raw PII/provider responses remain local-only in `/opt/ares/lead-data/email_marketing_herrington_browne_2026-05-18/`.
- Created an owned-number SMS smoke lead at Martin's request, sent one TextGrid test SMS through the Ares SMS service with a process-local live-send override, confirmed provider status `delivered`, and started a temporary allowlisted max-turn qualification watcher for Martin's number only. Global auto-replies remain disabled. No seller email send, Instantly upload/activation, or broad SMS setting change occurred. QC: `docs/qc/2026-05-18/email-marketing-herrington-browne-prep/`.
