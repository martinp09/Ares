...
- Probate autopilot is a live no-send intelligence/enrichment system: VPS `/opt/ares/Ares` is deployed from current `origin/main`/`17594ba`, Docker `ares-api` has durable `/var/lib/ares/lead-machine` mounted, SMS reply-agent runtime is live in draft-only/no-auto-reply mode, Slack notifications are configured, and Hermes no-agent cron `815e1261ab2e` remains the active no-send CT scheduler/watchdog until one controlled Trigger no-send run is proven. Trigger CLI auth is recovered for Hermes, Trigger prod `20260516.2` is deployed with cloud schedule no-op guards, Trigger prod env now targets the current Ares API through Tailscale Funnel `https://ares.tail485fd9.ts.net`, and protected probate health passes with `no_send_ok=true` / `outbound_allowed=false` while `ARES_TRIGGER_SCHEDULES_ENABLED=false` stays explicit to avoid duplicate runs. Dedupe/isolation hardening uses `county_case_sha256_v1` hashed identities scoped by `business_id + environment + source_run_scope`; scheduled payloads are `autonomous`; manual Hermes forced runs use isolated manual state/environment; remote Supabase migration `20260516131500_probate_source_identity_dedupe.sql` is applied; and `NightlyLeadMachineService` reads/writes `public.probate_source_identities` when `LEAD_MACHINE_BACKEND=supabase`. Post-adapter monitor passed 2-day live no-send smoke with 48 source rows/8 keep-now/no sends; Harris live rows are postback-only for case-detail and safely incomplete (`case_detail_postback_only`). QC: `docs/qc/2026-05-16/vps-current-main-rebuild-trigger-funnel/`, `docs/qc/2026-05-16/probate-production-readiness-wrap/`, `docs/qc/2026-05-16/probate-post-adapter-live-no-send-monitor/`, `docs/qc/2026-05-16/probate-dedupe-runtime-isolation/`, `docs/qc/2026-05-16/probate-source-identity-supabase-migration/`, `docs/qc/2026-05-16/probate-source-identity-supabase-adapter/`, and `docs/qc/2026-05-16/slack-routing-vps-deploy/`.
- Slack notification routing originally merged to `main` as `ff7dd9b` and is included in the current VPS deploy at `17594ba`. Remote Supabase migration `20260516012000_slack_notifications.sql` is applied; Slack route channels now exist; the Ares Slack bot is installed with `chat:write` and `chat:write.public`; `/opt/ares/Ares/.env` has `SLACK_NOTIFICATIONS_ENABLED=true`, `SLACK_BOT_TOKEN`, and all five route channel IDs; `ares-api` was rebuilt/recreated and health checks pass. Readiness is configured with all five requested routes `would_post=true`, and Slack `auth.test` returns `ok=true` for team `Ares`. Verification before merge/deploy: focused `224 passed`, full backend `1050 passed`, Trigger typecheck, Docker CI, and `git diff --check` passed. No Slack posts were made.
...
- `/opt/ares/worktrees/ares-main` on `main` includes the Harris+Montgomery probate autopilot PRD execution as an operational no-send system: code commit `17594ba` is deployed on the VPS; production no-send env preflight is healthy for `limitless/prod`; scheduled/background intelligence can use live public sources, live public case-detail evidence attempts, and live public CAD/tax/land enrichment; runtime requests still require no-send approvals while all outbound/provider-send paths remain blocked. Trigger CLI auth is recovered, prod version `20260516.2` is deployed, and Trigger prod env now points at `https://ares.tail485fd9.ts.net`; schedules remain no-op with `ARES_TRIGGER_SCHEDULES_ENABLED=false` until one controlled Trigger no-send run is proven, so Hermes no-agent cron `815e1261ab2e` remains authoritative for now.
...
- Trigger project `proj_puouljyhwiraonjkpiki` has prod version `20260516.2` deployed with schedule guards; prod runtime env targets `https://ares.tail485fd9.ts.net`, runtime key fingerprint matches the VPS key, and `ARES_TRIGGER_SCHEDULES_ENABLED=false` remains explicit until the controlled Trigger no-send promotion.
...
1. Run one controlled Trigger no-send run against `https://ares.tail485fd9.ts.net`; if it proves the expected no-send lifecycle without duplicate source runs, set `ARES_TRIGGER_SCHEDULES_ENABLED=true` and pause Hermes cron `815e1261ab2e`.
...
- Advanced `/opt/ares/Ares` from `b74d495` to current `origin/main`/`17594ba`, backed up `/opt/ares/Ares/.env`, added explicit SMS reply-agent draft/no-auto-reply env defaults, then rebuilt/recreated Docker `ares-api` and `ares-ui` from that commit.
