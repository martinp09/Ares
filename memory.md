...
- Probate autopilot is a live no-send intelligence/enrichment system: Trigger schedules and backend defaults run Harris+Montgomery public probate source adapters, public case-detail evidence enrichment, and public CAD/tax/land-record enrichment, while Ares still requires explicit no-send approvals and blocks Instantly/SMS/Vapi/paid skiptrace/HubSpot batch writes until separately approved. Dedupe/isolation hardening: source rows use `county_case_sha256_v1` hashed identities, prior-run comparisons are scoped by `business_id + environment + source_run_scope`, scheduled payloads are `autonomous`, manual Hermes forced runs use isolated manual state/environment, remote Supabase migration `20260516131500_probate_source_identity_dedupe.sql` is applied for the durable identity ledger, and `NightlyLeadMachineService` now reads/writes `public.probate_source_identities` when `LEAD_MACHINE_BACKEND=supabase`. Post-adapter live no-send monitor QC `docs/qc/2026-05-16/probate-post-adapter-live-no-send-monitor/`: env preflight blocked until durable production state/artifact/business/environment vars are configured; 2-day smoke passed with 48 source rows/8 keep-now/no sends; Harris live rows are postback-only for case-detail and are now classified incomplete (`case_detail_postback_only`) instead of blocked. QC `docs/qc/2026-05-16/probate-dedupe-runtime-isolation/`, `docs/qc/2026-05-16/probate-source-identity-supabase-migration/`, and `docs/qc/2026-05-16/probate-source-identity-supabase-adapter/`. 2026-05-16 scheduler fix: 07:10 CT now pulls previous-day→current-day, 02:20 pulls 7 days, Sunday 03:15 pulls 30 days, and zero-row county result pages are valid non-errors; temp replay parsed Harris `40` + Montgomery `8`, SLA healthy. QC `docs/qc/2026-05-16/probate-autopilot-scheduler-runtime-error/`.
...
- VPS `100.74.177.6` live Ares wiring as of 2026-05-16: Caddy is bound to tailnet `100.74.177.6:80` only and routes API paths to Docker `ares-api` on `127.0.0.1:8000` and UI fallback to `ares-ui` on `127.0.0.1:8080`; `/opt/ares/docker-compose.yml` builds both images from `/opt/ares/Ares`; `/opt/ares/Ares` is detached at `origin/main`/`32a3f57`; API/UI containers run non-root with `no-new-privileges`, dropped caps, and loopback-only published ports; Caddy loads the runtime bearer from root-only `/etc/caddy/ares-runtime.env`; `ares-edge-firewall.service` drops public `eth0` access to Caddy/Supabase dev ports; Supabase migration `20260516011000_deal_spine_runtime` is applied. Post-deploy smoke: direct API `/deals` without auth `401`; tailnet `/health` 200, `/deal-desk` 200, `/deals` 200 in 327ms, `/deals/fire-list` 200 in 73ms, `/mission-control/probate-autopilot/health` 200. QC: `docs/qc/2026-05-16/vps-edge-container-hardening/`.
- HubSpot operating spine / agentic company Phases 1-9 are complete with final QC index/readiness artifacts and runbooks under `docs/qc/2026-05-14/` and `docs/runbooks/`.
