...
- Probate autopilot is a live no-send intelligence/enrichment system: VPS `/opt/ares/Ares` is deployed from `fc99b75`, production no-send env preflight is healthy for `limitless/prod`, Docker `ares-api` has durable `/var/lib/ares/lead-machine` mounted, and Hermes no-agent cron `815e1261ab2e` is the active no-send CT scheduler/watchdog until Trigger.dev CLI auth is recovered. Trigger code/schedules typecheck but cloud deploy is blocked by login/auth. Dedupe/isolation hardening uses `county_case_sha256_v1` hashed identities scoped by `business_id + environment + source_run_scope`; scheduled payloads are `autonomous`; manual Hermes forced runs use isolated manual state/environment; remote Supabase migration `20260516131500_probate_source_identity_dedupe.sql` is applied; and `NightlyLeadMachineService` reads/writes `public.probate_source_identities` when `LEAD_MACHINE_BACKEND=supabase`. Post-adapter monitor passed 2-day live no-send smoke with 48 source rows/8 keep-now/no sends; Harris live rows are postback-only for case-detail and safely incomplete (`case_detail_postback_only`). QC: `docs/qc/2026-05-16/probate-production-readiness-wrap/`, `docs/qc/2026-05-16/probate-post-adapter-live-no-send-monitor/`, `docs/qc/2026-05-16/probate-dedupe-runtime-isolation/`, `docs/qc/2026-05-16/probate-source-identity-supabase-migration/`, and `docs/qc/2026-05-16/probate-source-identity-supabase-adapter/`.
- `feature/slack-notification-routing-current-main` ports Slack notification routing onto current `origin/main` (`0fc3f80`) after the original `feature/slack-notification-routing` branch became stale at base `247e8a2`. It adds persisted Slack attempts plus routes for lead-run digests, hot enriched leads, Instantly replies, lease-option inbound leads, SMS replies, and Vapi call events. Read-only VPS/GitHub check found no open PRs, live `/opt/ares/Ares` at `fc99b75`, `/opt/ares/worktrees/ares-main` at `0fc3f80`, healthy API/UI containers, and no Slack env vars configured. Verification: focused `224 passed`, full backend `1050 passed`, Trigger typecheck passed, and `git diff --check` passed. No Slack sends, env edits, provider sends, deploys, Supabase mutations, or VPS mutations were made.
...
- `/Users/solomartin/Projects/Ares/.worktrees/feature-slack-notification-routing-current-main` is on `feature/slack-notification-routing-current-main` from current `origin/main`; it ports Slack notification routing from stale `feature/slack-notification-routing` without dropping probate/VPS hardening. It implements persisted Slack attempts, readiness checks, route config, and notifications for lead-machine run digests/hot leads, Instantly replies, lease-option inbound leads, SMS replies, and Vapi call events. Activation still requires Slack bot token/channel IDs, migration application, and an intentional deployment.
...
12. Activate Slack routing only after Slack bot token/channel IDs are configured, `20260516012000_slack_notifications.sql` is applied, and a deliberate deploy/promotion is approved.
