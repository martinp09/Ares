...
- The same branch now includes the Mission Control operator UI refresh and analytics segmentation: the visible dashboard is a real-estate operator cockpit inspired by builderz Mission Control styling and `builderz-labs/marketing-dashboard`; primary nav foregrounds Today Desk, replies/submissions, approvals, to-do, blocked/dead, source health, and deal flow; the overview uses a KPI strip, graph-style lane performance, contact-mix donut, acquisition funnel, blocker chart, and segment cards; backend/admin surfaces (agents/catalog/runs/settings) are hidden from the primary desk and require the deliberate command/search unlock `backstage`. QC: `docs/qc/2026-05-18/ares-mission-control-operator-ui-refresh/` and `docs/qc/2026-05-18/ares-dashboard-analytics-segmentation/`.
