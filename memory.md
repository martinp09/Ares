...
- `/opt/ares/worktrees/ares-main` currently has `fix/probate-autopilot-enrichment-wiring` active for the Harris+Montgomery probate autopilot follow-up.
- Probate autopilot is a no-send intelligence/enrichment system first: source-run lanes/fields support Harris+Montgomery probate; manifests, morning brief sections, CT Trigger schedules, optional file-backed idempotency/source rows, source-file payload CLI, read-only doctor, Mission Control health surface, disabled-by-default source-provider bridge, live Harris/Montgomery source adapters, and inline nightly property/CAD + tax-overlay + land-record/title-friction enrichment for keep-now rows all stay behind explicit no-send gates. 2026-05-15 manual pilot proved Harris and Montgomery public source reads with no provider side effects; follow-up enrichment wiring QC lives at `docs/qc/2026-05-15/probate-autopilot-enrichment-wiring/`.
