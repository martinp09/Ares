...
export default function App() {
  ...
  const shellFallbackViews = fallbackViews.filter((view) => view !== "probate-autopilot");
  const fallbackLabel = shellFallbackViews.length > 0 ? ` (${shellFallbackViews.join(", ")})` : "";
  const isFullFixtureMode = shellFallbackViews.length === 10;
  ...
  const footerNote = isLoading
    ...
    : dataSource === "api"
      ...
      : isFullFixtureMode
        ...
        : `Using fixture fallback for: ${shellFallbackViews.join(", ")}.`;
  const scopeControls = (
    <OrgSwitcher
