...
2. Next source-provider gate: implement real Harris source adapter/browser bridge and Montgomery source discovery/adapter only behind explicit live-source approval gates; current source-provider bridge supports local export files only via `source_provider_bridge.mode=local_export_files`, and `LEAD_MACHINE_LIVE_SOURCE_CALLS_ENABLED` remains false by default.
...
- Probate autopilot next phase: implement real Harris and Montgomery source adapters behind the no-send source-provider gate; until then use either `source_provider_bridge.mode=local_export_files` in scheduled source-pull metadata or the source-file CLI as the interim bridge for exported rows; preserve raw-first artifacts, idempotency, expected-counties SLA, source-count mismatch warnings, Mission Control Autopilot health visibility, and aggregate duplicate-case redaction.
