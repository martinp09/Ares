...
status: active
updated_at: "2026-05-18T22:46:21Z"
repo: "martinp09/Ares"
local_checkout: "/opt/ares/worktrees/ares-main"
target_branch: "main"
merged_feature_branch: "feature/ares-chief-of-staff-v0"
chief_of_staff_code_commit: "82764fb"
...
5. Appointment Setter Slack loop: configure `SLACK_CHANNEL_APPOINTMENT_SETTER`; add Slack-safe hot-seller / needs-Martin-review alerts and approve/reject/takeover commands that write manager intent through Ares only. Slack commands must not call TextGrid, calendar, Instantly, HubSpot, or provider APIs directly.
6. Calendar foundation: add Cal.com-first availability/action adapter behind Ares policy gates; Appointment Setter receives sanitized availability/options only and never gets calendar credentials.
7. SMS policy: keep `SMS_AGENT_AUTO_REPLIES_ENABLED=false`, use `SMS_AGENT_ALLOWED_FROM_NUMBERS` for owned-number smokes, keep `APPOINTMENT_SETTER_ENABLED` scoped, and require explicit approval/receiver/max-turn/expiry gates before any broader live auto-reply.
8. Email marketing remains separate: review the Herrington/Browne draft campaign, sanitized QC, and local-only raw verification/contact artifacts before approving exact contacts, copy, Instantly upload/enrollment, daily cap, sender, or seller send.
9. Keep Trigger probate schedules authoritative (`07:10`, `12:40`, `17:40` CT) and Hermes cron `815e1261ab2e` paused unless intentionally rolling back.
10. Keep Instantly enrollment/send, SMS/Vapi dispatch, paid skiptrace, HubSpot batch mirror writes, and live deploy/promotions gated until separately approved.
