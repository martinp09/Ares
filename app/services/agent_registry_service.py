...
class AgentRegistryService:
    ...
    def get_agent(self, agent_id: str, *, org_id: str | None = None) -> AgentResponse | None:
        ...
        if agent is None or (org_id is not None and agent.org_id != org_id):
            return None
        revisions = agents_repository.list_revisions(agent_id)
    ...
    def list_agents(
        ...
        *,
        ...
        return self._agents_repository().list_agents(org_id=org_id, business_id=business_id, environment=environment)

    def get_agent_revision_state(self, agent: AgentRecord) -> AgentRevisionState | None:
        agents_repository = self._agents_repository()
        if agent.active_revision_id is not None:
            revision = agents_repository.get_revision(agent.active_revision_id)
    ...
    def promote_revision_to_candidate(
        ...
        *,
        ...
        if agent is None or (org_id is not None and agent.org_id != org_id):
            return None
        result = agents_repository.promote_revision_to_candidate(agent_id, revision_id)
        if result is None:
            return None
        ...
        return AgentResponse(agent=agent, revisions=agents_repository.list_revisions(agent_id))
    ...
    def publish_revision(self, agent_id: str, revision_id: str, *, org_id: str | None = None) -> AgentResponse | None:
        ...
        if agent is None or (org_id is not None and agent.org_id != org_id):
            return None
        result = agents_repository.publish_revision(agent_id, revision_id)
        if result is None:
            return None
        agent, revision = result
        self.audit.append_event(
    ...
    def archive_revision(self, agent_id: str, revision_id: str, *, org_id: str | None = None) -> AgentResponse | None:
        ...
        if agent is None or (org_id is not None and agent.org_id != org_id):
            return None
        result = agents_repository.archive_revision(agent_id, revision_id)
        if result is None:
            return None
        agent, revision = result
        self.audit.append_event(
    ...
    def get_revision_provider_kind(self, revision_id: str) -> ProviderKind | None:
        revision = self._agents_repository().get_revision(revision_id)
        if revision is None:
            return None
    ...
    def clone_revision(self, agent_id: str, revision_id: str, *, org_id: str | None = None) -> AgentResponse | None:
        ...
        if agent is None or (org_id is not None and agent.org_id != org_id):
            return None
        result = agents_repository.clone_revision(agent_id, revision_id)
        if result is None:
            return None
        agent, revision = result
        self.audit.append_event(
