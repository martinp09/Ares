...
class ProbatePropertyTaxTitleEnrichmentService:
    ...
    def run_enrichment(
        ...
        *,
        ...
        if live_cad_calls or live_tax_calls or live_land_record_calls:
            raise RuntimeError(
                "probate property/tax/title enrichment only accepts supplied local candidates; live CAD/tax/land-record calls are not enabled"
            )
        ...
        for row in keep_now_rows:
            ...
            candidates = hcad_candidates_by_case.get(record.case_number) or hcad_candidates_by_case.get(case_key) or []
            ...
            tax_payload = _tax_payload(tax_overlay)
            ...
            land_rows = land_record_rows_by_case.get(record.case_number) or land_record_rows_by_case.get(case_key) or []
            title_friction = _title_friction_payload(land_rows)
...
def _tax_payload(result: TaxOverlayResult | Mapping[str, Any] | None) -> dict[str, Any]:
    if result is None:
        return {
...
def _title_friction_payload(rows: list[Mapping[str, Any]]) -> dict[str, Any]:
    if not rows:
        return {
            "status": "not_checked",
            "instrument_count": 0,
