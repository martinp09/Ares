...
- 2026-05-16: Hardening cleanup on `fix/origin-main-hardening-cleanup`: dynamic Montgomery PublicSearch land-record end date, live no-send smoke asserts case-detail calls, legacy HubSpot CRM live writes require `operator_approval=true`, deployment Dockerfiles are tracked, and `.github/workflows/ci.yml` covers backend/Mission Control/Trigger/Docker gates.
