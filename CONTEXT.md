...
## Current TODO
1. Run post-merge local QC after reconciling TextGrid SMS agent with Slack activation docs.
2. Watch the next Hermes no-agent CT scheduler window for the first post-deploy `limitless/prod` autonomous morning brief.
3. Recover Trigger.dev CLI auth and deploy `trigger/` from current `main`, then pause Hermes autonomous scheduling to avoid duplicate source runs.
