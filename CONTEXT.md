...
## Current Scope
- Harris + Montgomery probate autopilot PRD is merged to `main` at `9c256bf` as a live, operational, no-send system.
...
## Current TODO
1. If production deployment is requested, configure durable state/artifact paths first: `LEAD_MACHINE_SOURCE_RUNS_STATE_PATH` and `LEAD_MACHINE_ARTIFACT_ROOT`.
2. Monitor no-send cron reports after deployment for aggregate source-run/enrichment health.
3. Keep all outbound/provider-send controls blocked until Martin approves exact recipients/campaigns.
4. Add stronger property matching once probate rows carry applicant/heir/property context.

## Recent Change
- 2026-05-15: Executed PRD as live no-send operational system and fast-forwarded it to `origin/main` at `9c256bf`; deleted the finished `fix/probate-autopilot-enrichment-wiring` branch.
