...
8. For SMS: use the temporary owned-number smoke transcript only for Martin's approved number; do not enable global SMS auto-replies until a scoped production approval/receiver exists. The branch now mirrors the Vapi pattern for SMS: deterministic safety/action policy, persisted conversation retrieval, optional LLM copy rewriting only, and Slack/operator handoff boundaries.
