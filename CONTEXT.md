...
## Recent Change
- 2026-05-10 production fix: lease-options landing `main` now includes the Ares-backed contact route (`martinp09/lease-options-landing` commit `455fe77`); public production no longer returns the old Supabase/n8n `500 {"error":"Submission failed"}` path. Current production response is explicit `502 ARES_INTAKE_FAILED` with `reason=runtime_not_configured` and missing Vercel envs: `BUSINESS_RUNTIME_MARKETING_LEADS_URL`, `BUSINESS_RUNTIME_API_KEY`, `BUSINESS_RUNTIME_BUSINESS_ID`, `BUSINESS_RUNTIME_ENVIRONMENT`. Vercel auth is unavailable in this shell.
